...
async function getPdfNamesAndUrls(page, date, currentUsername) { 
    ...
    for (const span of spans) {
        ...
        let spaceReplacement = '_'; // default space replacement is underscore
        let underscoreName;

        // Handle specific cases
        if (name.includes("GIP-PORC")) {
        ...
        } else if (name.includes("Etch")) {
            directory = 'TheftDeterrent';
        } else if (name.includes("VscRefund")) {
            directory = 'VscRefund';
        ...
        if (name.includes("Narrowgate Coldfront") || name.includes("Lion Crusader")|| name.includes("KCJ & S McCluskey Fam")) {
            ...
            spaceReplacement = '%20';  
            underscoreName = name.replace(/ /g, spaceReplacement);
            
            // Check if it's not the special "KCJ & S McCluskey Fam" case
            if (!name.includes("KCJ & S McCluskey Fam")) {
                // Now, replace only the last space with _
                const lastSpaceIndex = underscoreName.lastIndexOf(spaceReplacement);
                if (lastSpaceIndex !== -1) {
                    underscoreName = underscoreName.slice(0, lastSpaceIndex) + "_" + underscoreName.slice(lastSpaceIndex + spaceReplacement.length);
                }
            }
            // Now, replace only the last space with _
            const lastSpaceIndex = underscoreName.lastIndexOf(spaceReplacement);
            ...
            if (name.endsWith("LP-PORC.pdf")) { // New condition for LP-PORC.pdf for lion crusader only
              underscoreName = "LP-PORC_A.pdf";
            }
            if (name.endsWith("VESC-SC-PORC.pdf")) { // another dumb condition found in lion crusader
              underscoreName = "VESC-SC-PORC_A.pdf";
            }
